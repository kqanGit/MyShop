    ...
    public class ProductService : IProductService
    {
        ...
        public async Task<PagedResult<ProductDto>> GetProductsPaged(int pageIndex, int pageSize, int? categoryId, decimal? maxPrice, bool? DESC)
        {
            ...
            var products = await query
                ...
                .Select(p => new ProductDto
                {
                    ...
                    Price = p.Price,
                    Stock = p.Stock,
                    Image = p.ProductImages.Where(pi => pi.IsPrimary).Select(pi => pi.ImageUrl).FirstOrDefault() ?? p.Image,
                    IsRemoved = p.IsRemoved == true,
                    Images = p.ProductImages.Select(pi => new ProductImageDto
                    {
                        ImageIndex = 0, // Legacy/unused
        ...
        public async Task<ProductDto> GetProductById(int productId)
        {
            ...
            return new ProductDto
            {
                ...
                Price = product.Price,
                Stock = product.Stock,
                Image = product.ProductImages.FirstOrDefault(pi => pi.IsPrimary)?.ImageUrl ?? product.Image,
